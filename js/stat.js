...
var BAR_WIDTH = 40;
var barHorizontal = CLOUD_Y + GAP * 2 + (FONT_GAP) * 3 + GAP + MAX_BAR_HEIGHT;
...
window.renderStatistics = function (ctx, players, times) {
  ...
  for (var i = 0; i < players.length; i++) {
    ...
    ctx.fillStyle = '#000';
    ctx.fillText(players[i], barX, barHorizontal + FONT_GAP);
    ctx.fillText(Math.round(times[i]), barX, barHorizontal - barHeight - GAP);
    ...
    ctx.fillRect(barX, barHorizontal - barHeight, BAR_WIDTH, barHeight);
  }
};
